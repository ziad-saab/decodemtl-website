...
export const immersiveFAQ = {
    en: [
        ...
        {
            question: 'Why full-stack JavaScript?',
            answer: (<div><p>As full-stack web developers, you will come to write both server-side and browser-side code. Web browsers understand mainly three languages: HTML, CSS and JavaScript. If you're going to add any interactivity to web pages, then JavaScript is the only way to go. While there exist other languages that compile to JavaScript, popular libraries like jQuery, Backbone, Angular and React have ensured that JavaScript remains the language of choice for building web applications today.</p><p>As far as server-side development goes -- the part that happens in the background, away from the user's computer -- there are many choices of languages and platforms. Ruby on Rails, Python with Django, PHP with Laravel, Java with Play Framework, etc. Choosing to learn JavaScript with NodeJS for server-side development has many advantages. As a modern web developer, you have to constantly juggle with different parts of the stack at the same time. Having a unified language eases this constant context-switching, and gives us more time to focus on learning interesting things.</p><p>Learning JavaScript does not limit you to the world of web development anymore. Frameworks such as React Native allow you to use JavaScript and a variant of CSS to build native iOS and Android applications. Platforms such as the Tessel Board allow you to write NodeJS-style code to retrieve data from real-world sensors and send commands to actuators, enabling you to prototype home automation solutions with JavaScript.</p></div>)
        },
        {
