import rachel from '../../../assets/images/testimonial-rachel-pariente.jpg';
import myriam from '../../../assets/images/testimonial-myriam-gariepy.jpg';

export default {
    en: [
        {
            fullName: 'Rachel Pariente',
            text: 'The prof communicates the material in a clear and articulate manner despite the complexity of the subject at hand. The space at WeWork is rather pleasant too. I must say, it gave me the yearning to become an entrepreneur!',
            img: rachel
        },
        {
            fullName: 'Myriam Gariépy',
            text: '10 weeks ago, I didn\'t even know how to open a zip file! Now, I can make a multi-page, responsive website with forms, links, videos, nice design, etc!',
            img: myriam
        },
    ],
    fr: [
        {
            fullName: 'Rachel Pariente',
            text: 'Le prof communique le matériel d\'une manière claire et articulée malgré la complexité du sujet en question. L\'espace de travail à WeWork est très agréable aussi. Je dois avouer, l’atmosphère m\'a donné l\'envie de devenir un entrepreneur!',
            img: rachel
        },
        {
            fullName: 'Myriam Gariépy',
            text: 'Il y a 10 semaines, je ne savais même pas comment ouvrir un fichier zip! Maintenant, je peux construire un site web à plusieurs pages avec des formulaires, des liens, des vidéos, des nouvelles conceptions, etc!',
            img: myriam
        },
    ]
}